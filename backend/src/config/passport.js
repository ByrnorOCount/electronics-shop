...
const findOrCreateUser = async (provider, profile, done) => {
    ...
    if (!email) {
        return done(new Error(`'${provider}' did not return an email address.`), null);
    }

    try {
        // 1. Find a user by their provider ID.
        let user = await db('users').where({ provider, provider_id: providerId }).first();
        if (user) {
    ...
    new GoogleStrategy(
        {
            ...
            // This is the callback URL you set in the Google Cloud Console
            callbackURL: '/api/auth/google/callback',
            proxy: true, // Allow proxy (important for deployment)
        },
